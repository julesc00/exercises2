"""
This implementation demonstrates how to efficiently determine
if the opening and closing brackets of an arithmetic
...
Example:
    ...
    Let n be the number of tokens that make up the arithmetic expression.
...
def validate_expression(expression):
    ...
    for token in expression:
        ...
        if token in opening_symbols:
            ...
            # Closing symbol cannot be empty
            if is_empty(stack):
                return False
            if stack[-1] == matching_symbols[token]:
                # If closing symbol match opening at the top of stack
                # pop the top element
                stack.pop()
            else:
